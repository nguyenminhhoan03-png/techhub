...
class PercentageCalculatorTool implements ToolContract
{
    ...
    public function execute(array $input): ToolResult
    {
        ...
        if ('percent_of' === $mode) {
            ...
            $description = "{$valA}% of {$valB} is " . round($result, 4);
        } elseif ('is_what_percent' === $mode) {
            // X is what percent of Y? (e.g. 30 is what % of 150 = 20%)
            if (0.0 === $valB) {
                return ToolResult::failure('Cannot divide by zero for percentage calculation.', 0);
            }
            $result = ($valA / $valB) * 100;
            $description = "{$valA} is " . round($result, 4) . "% of {$valB}";
        } elseif ('increase_decrease' === $mode) {
            // Percentage increase/decrease from A to B
            if (0.0 === $valA) {
                return ToolResult::failure('Initial value cannot be zero for change calculation.', 0);
            }
            $diff = $valB - $valA;
            $result = ($diff / $valA) * 100;
            $changeType = $diff >= 0 ? 'increase' : 'decrease';
            $description = "Change from {$valA} to {$valB} is a {$changeType} of " . abs(round($result, 4)) . '%';
        }
